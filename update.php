...
$name_err = $description_err = $price_err = $discount_price_err = $status_err = "";
...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($name_err) && empty($description_err) && empty($price_err) && empty($discount_price_err) && empty($status_err) ){
        // Prepare an update statement
        $sql = "UPDATE items SET name=?, description=?, price=?, discount_price=?, modified=NOW(), status=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssi", $param_name, $param_description, $param_price, $param_discount_price ,$param_status, $param_id);
